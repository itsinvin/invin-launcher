...
mv target/x86_64-pc-windows-msvc/release/quartz_launcher dist/QuartzLauncher-Windows-x86_64.exe
...
'  "resources": [],'\
'  "authors": ["Moulberry"],'\
'  "binaries": [{ "path": "QuartzLauncher-Windows-x86_64.exe", "main": true }],'\
'  "icons": ["package/windows.ico"]'\
'}'

mv -f dist/QuartzLauncher-Windows-x86_64.exe dist/QuartzLauncher-Windows-x86_64-Portable.exe
mv -f 'dist/QuartzLauncher-Windows-x86_64_'$version'_x64-setup.exe' dist/QuartzLauncher-Windows-x86_64-Setup.exe
...
if [[ -n "$CARGO_PACKAGER_SIGN_PRIVATE_KEY" ]]; then
    ...
    \"downloads\": {
        \"x86_64\": {
            \"executable\": {
                \"download\": \"https://github.com/itsinvin/QuartzLauncher/releases/download/v$version/QuartzLauncher-Windows-x86_64-Portable.exe\",
