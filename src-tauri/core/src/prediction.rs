...
fn build_recommendations(
    ...
    if !w.optimization_mods {
        recs.push(Recommendation {
            ...
            detail: if fabric_family {
                "Sodium (rendering), Lithium (game logic) and FerriteCore (memory) typically more than double FPS and cut RAM use. invin can add them in one click.".into()
            } else {
    ...
    recs.push(Recommendation {
        ...
        detail: "invin can apply tuned G1GC flags (Aikar-style) for smoother frame times. Enabled by default for new instances.".into(),
        estimated_fps_gain: None,
    });
